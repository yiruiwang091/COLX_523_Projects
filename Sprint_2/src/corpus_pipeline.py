...
- (optional) export JSONL -> JSON array for Label Studio (--export-json)
...
- Use --reset to delete existing output + checkpoint (and exported JSON if enabled) and rebuild from scratch.

Selected fields:
- Review: review_id, asin, overall, reviewText
- Metadata: asin, title, price, description, rank, imageURL, cat_l1–cat_l5

Run (full corpus, default output -> Sprint_2/data):
    python Sprint_2/src/corpus_pipeline.py --brand Coleman --resume --export-json

Run (small test):
    python Sprint_2/src/corpus_pipeline.py --brand Coleman --limit 1000 --resume --export-json
...
def reset_outputs(out_jsonl: Path, state_path: Path, export_json: Optional[Path] = None) -> None:
    """Delete output and checkpoint (and optional exported JSON) to rebuild from scratch."""
    if out_jsonl.exists():
        out_jsonl.unlink()
    ...
    if export_json is not None and export_json.exists():
        export_json.unlink()
        print(f"[DEL ] Removed {export_json}")
...
def jsonl_to_json_array(jsonl_path: Path, json_path: Path) -> int:
    """
    Convert JSONL -> JSON array in a streaming way (no full-file memory),
    and write atomically to avoid corrupt outputs if interrupted.

    Returns number of items written.
    """
    if not jsonl_path.exists():
        raise FileNotFoundError(f"Missing JSONL input: {jsonl_path}")
    ...
    print(f"[CONV] JSONL -> JSON array: {n} items -> {json_path}")
    return n
...
def main() -> None:
    # Compute default output dir relative to this script:
    # Sprint_2/src/corpus_pipeline.py -> default out = Sprint_2/data
    SCRIPT_DIR = Path(__file__).resolve().parent
    SPRINT2_DIR = SCRIPT_DIR.parent
    DEFAULT_OUT = SPRINT2_DIR / "data"
    ...
    ap.add_argument(
        ...
        help="Base output dir (default: Sprint_2/data)",
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=200,
        help="Write checkpoint every N joined records (default: 200)"
    )

    # Export JSON array for Label Studio (keep JSONL as primary)
    ap.add_argument(
        "--export-json",
        action="store_true",
        help="After building JSONL, export to standard JSON array (Label Studio friendly)"
    )
    ap.add_argument(
        "--export-json-path",
        default=None,
        help="Optional explicit path for exported JSON (default: alongside JSONL, same basename .json)"
    )
    ...
    if args.export_json_path:
        export_json = Path(args.export_json_path)
    else:
        export_json = out_jsonl.with_suffix(".json")
    ...
    if args.reset:
        reset_outputs(out_jsonl, state_path, export_json if args.export_json else None)
    ...
    # 4) Stream join with resume (JSONL output)
    ...
    # 5) Export JSON array (Label Studio)
    if args.export_json:
        jsonl_to_json_array(out_jsonl, export_json)
